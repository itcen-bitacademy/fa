...
public class Menu44Service {
	...
	public DataResult<IntangibleAssetsVo> getList(int page, IntangibleAssetsVo intangibleAssetsVo, String startDate,
			String endDate) {
		DataResult<IntangibleAssetsVo> dataResult = new DataResult<>(); // 본인의 vo 형식을 리스트에 담음
		int totalCount = menu44Repository.selectCount(intangibleAssetsVo, startDate, endDate);
		...
		List<IntangibleAssetsVo> list = menu44Repository.getList(paginationUtil, intangibleAssetsVo, startDate, endDate);// 뽑은 list를 dataResult타입을 list에 세팅
																						// => 1개~ 11개 모두 리스트에 세팅
